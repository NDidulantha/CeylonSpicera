...
export default function AccountView() {
    ...
    const submit = async () => {
        ...
        try {
            if (mode === "login") {
                const r = await login(f.email, f.pw);
                if (r.ok) goBack();
                else setError(r.error);
                return;
            }
            const r = await register(f.name, f.email, f.pw);
            ...
            {user ? (
                <>
                    ...
                    <div className="mt-8 flex flex-wrap gap-3">
                        ...
                        <button type="button" onClick={async () => { await logout(); setNotice("You have signed out."); }} className="rounded-[25px] border px-8 py-3.5 font-sans text-[12px] font-semibold uppercase tracking-[0.16em] transition-colors" style={{ borderColor: "#C9B896", color: TERRA }}>Sign out</button>
                    </div>
                </>
            ) : (
                <>
                ...
                <span className="mt-0.5 flex h-[19px] w-[19px] flex-none items-center justify-center rounded-[4px] border" style={{ borderColor: "#C9B896", background: f.remember ? TERRA : "transparent" }}>
                  ...
                  I agree to the <a href="#" onClick={(e) => e.preventDefault()} className="underline" style={{ color: TERRA }}>terms of trade</a> and the <a href="#" onClick={(e) => e.preventDefault()} className="underline" style={{ color: TERRA }}>privacy notice</a>.
                    ...
                    {/* primary */}
                    <button type="button" disabled={!valid || submitting} onClick={valid ? submit : undefined} className="mt-7 w-full rounded-[25px] py-4 font-sans text-[12px] font-semibold uppercase tracking-[0.16em] transition-colors" style={{ background: valid ? "#1F3A2A" : "#E3D8C0", color: valid ? "#F7F3EA" : FAINT, cursor: valid && !submitting ? "pointer" : "not-allowed" }}>
                        {submitting ? "Please wait…" : valid ? (mode === "register" ? "Open an Account" : "Sign In") : mode === "register" ? "Complete the Entry" : "Enter Your Details"}
                    </button>
