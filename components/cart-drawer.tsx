...
import { CATALOG, SIZES, FREE_SHIP_AT, SHIP_FEE, money } from "@/lib/shop-data";
...
export default function CartDrawer() {
    ...
    const lines = Object.entries(cart)
        .map(([key, qty]) => {
            ...
            return { key, qty, p, size, lineTotal: p.price * size.mult * qty };
        })
        .filter(Boolean) as { key: string; qty: number; p: (typeof CATALOG)[number]; size: (typeof SIZES)[number]; lineTotal: number }[];
